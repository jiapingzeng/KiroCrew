...
Nothing here imports ctypes; window confirmation is :mod:`apps_windows`' job.
...
def _under_protected(path: str) -> bool:
    ...
    for root in _protected_roots():
        ...
        if folded == root_folded or folded.startswith(root_folded.rstrip(os.sep) + os.sep):
            if _directory_is_writable(os.path.dirname(real)):
                return False
            return not _file_is_writable(real)
    return False
...
def _file_is_writable(path: str) -> bool:
    ...
    **``os.open(.., O_RDWR)``, not ``os.access(.., W_OK)``.** On Windows ``os.access`` reports
    ...
    examined counts as writable, so an unverifiable target is refused rather than run.
    """
    try:
        fd = os.open(path, os.O_RDWR)
    except PermissionError:
        # The answer we want: the ACL refused write access.
        return False
...
def resolve_target(query: str) -> "tuple[str, str]":
    ...
    **The fuzzy tier is a PREFIX rather than a substring, for the same reason.** A
    ...
    app = matched[0]
    basename = os.path.basename(os.path.realpath(app.executable))
    ...
    if not _under_protected(app.executable):
    ...
    return app.executable, app.name
