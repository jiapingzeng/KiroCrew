...
async def _runtime_grant_present(mcp_url: str, name: str) -> bool | None:
    ...
    against a path that simply is not there, so ``grant_present`` returns
    ``False`` and an already-authorized server reads "Sign-in required". The
    consequence is mild and self-healing — the user signs in once more and the
    grant is re-minted — but it is a wrong-looking row, not a safe one, and the
    recorded-hash tests only pin this mirror against itself, so such a change
    would not fail in-repo.
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(server.url, json=init_body, headers=hdrs) as resp:
                if resp.status != 200:
                    ...
                    if _is_bearer_challenge(resp.headers.get("WWW-Authenticate", "")):
