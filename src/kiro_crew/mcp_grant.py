...
A leaf module on purpose. Two callers need this — ``connections.mint`` (the
curated-provider consent flow) and ``mcp_discovery``'s remote probe (any url a
user configured) — and each of the obvious alternatives is closed:
...
* Copying the key derivation into the second caller is worse still: it mirrors an
...
So the derivation lives here, where both callers reach it with an ordinary
...
async def grant_observed(mcp_url: str) -> bool:
    ...
    Audited on the TRUE result only, and deliberately NOT once per stat. The mint's
    watcher polls for up to its TTL, so a per-stat audit would write one
    synchronous event per poll for a single flow (``hooks.emit_internal_read_audit``
    marks the event critical so it drains the queue and cannot be silently lost). A
    negative poll observed nothing and changed nothing; the access that owes a
    trail is the one a caller ACTS on -- it moves a mint row to ``granted``, or it
    decides whether a probe reports a sign-in as required -- and that one is
    recorded.
