...
def test_only_the_post_insert_unwind_needs_the_gate():
    """Scope guard: the FOUR early rollbacks are unconditional on purpose.
    ...
    The fourth is the stale-decision-record refusal (round 21). It is reached after
    two awaits of its own, but both precede the insert, so this create has claimed
    nothing and ``created_worktree`` is still only ever the one it made itself --
    the same reasoning that leaves the other three ungated.
    """
    src = inspect.getsource(routes._handle_create)
    early = src[:src.index("async def _unwind_create")]
    assert early.count("_remove_worktree(") == 4, (
...
async def test_a_claim_through_an_aliased_spelling_is_refused(tmp_path, monkeypatch):
    ...
    empty record, exactly the alias hole. It cannot record anything, because the claim
    refuses a spec_dir that does not verify as itself.
    """
    _redirect_state(monkeypatch, tmp_path)
    ...
    routes._save_index(
        {
            "real": {**entry, "spec_dir": str(real), "slot_key": "spec-builder-real-aaaaaaaa"},
            "alias": {**entry, "spec_dir": str(link), "slot_key": "spec-builder-alias-bbbbbbbb"},
        }
    )
    ...
    assert store[routes._decision_key(str(real))]["answers"] == {"transport": "HTTPS"}
...
def test_alias_keys_are_resolved_not_trusted_from_the_index():
    ...
    assert 'meta.get("slot_key"' not in src, (
        "the alias scan trusts the agent-writable slot_key field"
    )
...
async def test_an_alias_with_no_slot_key_is_still_seen(tmp_path, monkeypatch):
    ...
    # It resolves to the key _ensure_worker_slot would run that name under.
    assert aliases == {"spec-builder-alias": "alias"}, aliases
...
async def test_an_alias_that_copies_our_key_is_still_seen(tmp_path, monkeypatch):
    ...
    assert aliases == {"spec-builder-alias": "alias"}, (
...
@pytest.mark.asyncio
async def test_a_decision_absent_from_state_is_still_answerable(tmp_path, monkeypatch):
    """Carve-out two, also structural: there is no replacement to mislock.

    An agent that dropped a decision from its state is the case
    _apply_recorded_answers already documents. Refusing here would also break
    answering a decision whose re-emission has not landed on disk yet -- the very
    gap the ledger exists to cover.
    """
    client = _make_client(monkeypatch, tmp_path)
