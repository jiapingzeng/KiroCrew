...
_AUDIT_ONLY_READ_IDS: dict[str, str] = {
    ...
    #
    # Audited on the observation a caller acts on, not per poll; see
    # ``mcp_grant.grant_observed`` for why that boundary is not fail-closed, and note
    # that neither caller logs the url itself (the probe logs the server name, the
    # mint warning logs the key) because a user-supplied endpoint can carry a
