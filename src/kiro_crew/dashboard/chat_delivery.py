...
async def steer_into_running_turn(
    ...
    if queued:
        ...
        return STEER_REQUEUED
    if stopped:
        # Absence alone does not say WHICH consumer took the registration. Two
        # things remove one: the running turn CONSUMING the steer, and the
        # hard-kill clear. They are told apart by the delivery id, because a
        # consume leaves `_steer_delivery_ids` populated while the hard kill drops
        # it along with the pending entry.
        if message not in slot._steer_delivery_ids:
            # Discarded by a hard kill: the turn it was written into is being
            # killed, so the text never runs. Reporting delivery here would
            # persist a transcript row for a message that did not execute and tell
            # the caller it landed. Resending is safe precisely because the kill
            # guarantees it did not run.
