...
class AcpRuntime:
    ...
    async def load_session(
        ...
        # — no awaited step, nothing to unwind, no shared coroutine that could
        # grow a failure mode later (harness-parity H13: the test is whether the
        # kiro path changed at all, not whether it still works).
        if self._acp_backend == ACP_BACKEND_KAS:
