...
| Review verbs | approve / request changes / comment | approve / comment (request changes REFUSED) | approve / request changes / comment (votes `10` / `-10` / `0`) |
...
that did not happen, would be worse than the error. Every refusal in this section is
GitLab's: GitHub and Azure DevOps both express the whole surface natively (see
[Providers](#providers)).
...
- POSIX only (macOS/Linux). Windows raises `GhCliError` immediately — on every
  provider; `azure_client._az_bin` refuses `win32` before it resolves anything.
...
- Any CLI the user can run from their terminal is accepted: the well-known dirs
  ...
  `KIROCREW_PROVIDER_BIN_STRICT=1`. Note that the shared candidate table has
  well-known-directory entries only for `gh` and `glab`, so under strict mode `az`
  resolves solely through its override, and the setup error says so rather than
  reporting a missing install.
