    ...
    *,
    ...
    with con:
        ...
        for key in auth_keys:
            ...
            con.execute(
                "INSERT INTO auth_kv (key, value) VALUES (?, ?)", (key, json.dumps(blob))
            )
        if state_rows:
            con.execute(
            ...
            con.execute(
                "INSERT INTO state (key, value) VALUES (?, ?)", ("api.codewhisperer.profile", profile)
            )
...
class TestIdentityFingerprint:
    ...
    def test_an_unauditable_read_never_opens_the_store(
    ...
    def test_a_failed_terminal_audit_discards_the_result(
        self, tmp_path: Path, monkeypatch
    ) -> None:
...
class TestFingerprintCaching:
    ...
    @pytest.mark.asyncio
    async def test_a_logout_inside_the_cache_window_is_still_detected(
        self, tmp_path: Path
    ) -> None:
...
class TestStorePathSelection:
    ...
    def test_windows_path_ignores_a_redirected_appdata(self, tmp_path: Path) -> None:
        ...
        path = kp.kiro_identity_store_path(
            "win32", tmp_path, {"APPDATA": str(tmp_path / "evil"), "LOCALAPPDATA": str(tmp_path / "evil")}
        )
...
class TestStoreRelocation:
    ...
    @pytest.mark.asyncio
    async def test_the_default_store_is_still_read_when_not_relocated(
        self, tmp_path: Path
    ) -> None:
...
class TestIdentityChangePredicate:
    ...
    @pytest.mark.asyncio
    async def test_a_logout_before_the_first_turn_is_still_detected(
        self, tmp_path: Path
    ) -> None:
...
class TestBaselinesAreIndependent:
    ...
    @pytest.mark.asyncio
    async def test_an_unreconciled_baseline_never_reads_as_matching(
        self, tmp_path: Path
    ) -> None:
