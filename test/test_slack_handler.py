...
class TestHandleMessage:
    ...
    @pytest.mark.asyncio
    async def test_tail_only_reply_is_not_booked_a_delivery_failure(self, monkeypatch):
        """A reply that is ENTIRELY a credential-class run is delivered by the
        tail flush alone: every streamed delta is withheld as a possible partial
        credential and confirms nothing, so the flush is the only append that
        lands text. It must count as a real delivery -- otherwise an ordinary
        one-word answer books the turn a failure and walks the consecutive-
        failure breaker toward a session reset."""
        import kiro_crew.slack.handler as _h
        ...
        slack = MockSlackClient()
        slack._stream_enabled = True
        # No trailing separator: the whole reply stays inside the credential-class
        # run, so nothing commits during streaming.
        provider = FakeProvider([LLMEvent(kind="text_chunk", text="42")])
        sessions = TrackingSessions(provider)
        await handle_message(slack, sessions, "C1", "q", None, "msg1", "U1")

        appended = "".join(a[1].get("text") or "" for a in slack.actions if a[0] == "append_stream")
        # The tail flush is what delivered the answer.
        assert "42" in appended
        # ...and the turn is not booked a failure for it.
        assert sessions.record_failure_calls == 0, slack.actions
        all_text = " ".join(
            str(a[1].get("text") or "")
            for a in slack.actions
            if a[0] in ("post", "update", "append_stream", "stop_stream")
        )
        assert "Something went wrong" not in all_text

    @pytest.mark.asyncio
    async def test_lost_pre_wait_tail_overwrites_the_stopped_message(self, monkeypatch):
        """A CLEAN pre-wait tail that lands on neither the append nor its rotated
        retry leaves the wire copy truncated, and the pre-wait message is stopped
        with `accumulated` reset -- no later render verdict covers those bytes.
        The stopped message must be overwritten with the complete pre-wait text,
        the same recovery the redacted verdict gets."""
        import kiro_crew.slack.handler as _h
        from kiro_crew.acp.types import EVENT_TOOL_CALL
        ...
        class TailRefusingSlack(MockSlackClient):
            """append_stream refuses every frame carrying the withheld tail."""
        ...
        updates = [a[1].get("text") or "" for a in slack.actions if a[0] == "update"]
        assert any("Result: 12345" in u for u in updates), slack.actions
