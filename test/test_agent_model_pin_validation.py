...
class TestDoctorFlagsSpecPins:
    ...
    def test_unreadable_specs_report_unchecked_not_clean(self, tmp_path, monkeypatch):
        """A diagnostic must not print green for a check it never performed."""
        from kiro_crew import cli_doctor

        def _boom(**_kw):
            raise OSError("agents dir on fire")

        monkeypatch.setattr(cli_doctor, "list_agents", _boom)

        result = cli_doctor._agent_spec_model_problems(agents_dir=tmp_path / "agents")
...
class TestNoRedundantConfigLoad:
    ...
    def test_supplied_provider_skips_the_load(self, monkeypatch):
        ...
        _model_rejected_reason("claude-opus-4-8", provider="kiro")
        assert calls == []
    ...
    def test_validator_forwards_the_provider(self, monkeypatch):
        ...
        _validate_role_model("claude-opus-4-8", request, provider="kiro")
        assert calls == []
...
class TestSavePathRefusesAnUnusablePin:
    ...
    async def test_create_refuses_and_carries_an_error_code(self, seeded_agent):
        ...
        async with TestClient(TestServer(_crud_app())) as client:
            resp = await client.post(
                "/api/agents", json={"name": "reviewer", "model": "claude-opus-4-8"}
            )
    ...
    async def test_a_usable_pin_is_accepted(self, seeded_agent):
        async with TestClient(TestServer(_crud_app())) as client:
            resp = await client.post(
                "/api/agents", json={"name": "good", "model": "claude-opus-4.8"}
            )
    ...
    async def test_auto_is_accepted(self, seeded_agent):
        async with TestClient(TestServer(_crud_app())) as client:
            resp = await client.post("/api/agents", json={"name": "inherits", "model": "auto"})
            assert resp.status == 200
