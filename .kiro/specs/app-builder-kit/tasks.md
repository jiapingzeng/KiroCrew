# Implementation Plan — KiroCrew App Builder Kit
...
- [ ] 1. Scaffold the kit module and lifecycle core
  ...
  - Implement `kit/tool-views/core/types.ts` (`ToolViewState`, `ToolViewDef`) and `defineToolView`
...
- [ ] 4. Dispatch and mcwidget fallback
  - Implement `ToolRenderer` mapping `tool-<name>` parts to registered `ToolViewDef`s
  - Integrate at `components/ContentRenderer.tsx`; unmatched parts fall through to existing `<mcwidget>` render with no regression
  - Integration test: matched → component, unmatched → `<mcwidget>`
...
- [ ] 8. Data and async helpers (kit/data)
  - `useAppResource<T>` (data/loading/error/refetch) over `api/client.ts`; `useAppMutation` (optimistic + rollback); `useAppLiveResource` on `useAppEvents` gated by `checkSubscribeAllowed`
