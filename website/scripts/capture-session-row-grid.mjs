...
async function main() {
  ...
  for (const theme of ['dark', 'light']) {
    ...
    const rows = await page.locator('.session-row').evaluateAll(els => els.map(el => {
      ...
      // The row's status marker: the FIRST child of the secondary line, which is
      // where it now lives (an `svg` glyph, or the unread dot's `span`).
      const marker = statusLine ? statusLine.firstElementChild : null
      ...
      return {
        ...
        edges: {
          ...
          title: rel(headline),
          status: statusLine ? rel(statusLine.getBoundingClientRect()) : null,
        },
        ...
        // marker leads the words it names, so it has to sit ON their line.
        glyphOffset: marker && statusLine
          ? Math.round(((marker.getBoundingClientRect().top + marker.getBoundingClientRect().height / 2)
            - (statusLine.getBoundingClientRect().top + statusLine.getBoundingClientRect().height / 2)) * 100) / 100
          : null,
        // Marker left edge relative to the row — it must clear the row's left-edge
        // decorations (recency tint up to 7px, colour bar 2px) by starting at the
        // content column, which is the row's whole `pl-3.5`.
        markerLeft: marker ? Math.round((marker.getBoundingClientRect().left - box.left) * 100) / 100 : null,
        contentLeft: Math.round((col.getBoundingClientRect().left - box.left) * 100) / 100,
