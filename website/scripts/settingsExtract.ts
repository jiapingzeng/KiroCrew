  ...
  for (const primitiveName of PRIMITIVES) {
    ...
    while ((tagStartMatch = tagStartRe.exec(source)) !== null) {
      ...
      for (const target of targets) {
        ...
        // label IS the rendered text and must stay byte-identical.
 ...
 */
export function mergeManualEntries(generated: SettingEntry[], manual: SettingEntry[]): SettingEntry[] {
  ...
  for (const m of manual) {
    ...
    seen.add(m.id)
    if (m.labelKey && getEnCatalog()[m.labelKey] === undefined) {
      throw new Error(
