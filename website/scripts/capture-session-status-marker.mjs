...
const slots = [
  ...
  },
  {
    ...
    // so it is the one that keeps an accessible name after the move.
    key: 'chat-unread', title: 'Terminal font picker round 7', running: false, messages: 24,
...
async function main() {
  ...
  for (const theme of ['light', 'dark']) {
    ...
    const rows = await page.locator('.session-row').evaluateAll((els, tintMax) => els.map(el => {
      const box = el.getBoundingClientRect()
      // Shape-agnostic: whichever element carries one of the marker glyphs, at
      // whatever depth. The list is explicit because the row holds OTHER svgs
      // (folder chip, provider logos, channel provenance) that are not the status
      // marker, and because the static glyphs (shield, question) carry no
      // animation class to find them by.
