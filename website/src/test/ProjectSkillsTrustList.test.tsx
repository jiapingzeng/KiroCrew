...
describe('ProjectSkillsTrustList', () => {
  ...
  it('lists each trusted folder', async () => {
    mockApi.skillTrust.mockResolvedValue({
      grants: [
        { path: '/home/kseam/repo-a', exists: true },
        { path: '/home/kseam/repo-b', exists: true },
      ],
    })
    render(<Harness />)
    expect(await screen.findByText('/home/kseam/repo-a')).toBeInTheDocument()
    expect(screen.getByText('/home/kseam/repo-b')).toBeInTheDocument()
  })
  ...
  it('keeps a grant visible after its folder is gone', async () => {
    ...
    mockApi.skillTrust.mockResolvedValue({
      grants: [{ path: '/home/kseam/deleted', exists: false }],
    })
    render(<Harness />)
    expect(await screen.findByText('/home/kseam/deleted')).toBeInTheDocument()
  ...
  it('withdraws the grant for the row that was clicked', async () => {
    ...
    render(<Harness />)
    await screen.findByText('/home/kseam/repo-a')
    const buttons = screen.getAllByRole('button', { name: /Withdraw/ })
    fireEvent.click(buttons[1])
    await waitFor(() =>
      expect(mockApi.revokeSkillTrust).toHaveBeenCalledWith('/home/kseam/repo-b'),
    )
  })
  ...
  it('explains what withdrawing does', async () => {
    mockApi.skillTrust.mockResolvedValue({
      grants: [{ path: '/home/kseam/repo-a', exists: true }],
    })
    render(<Harness />)
