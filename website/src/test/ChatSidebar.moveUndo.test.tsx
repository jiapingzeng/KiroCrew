...
describe('drag-to-folder undo', () => {
  ...
  it('undo posts the ORIGINAL folder back, then retires the offer', async () => {
    ...
    // null, not '' — the move hook's contract for "no folder". The third
    // argument is the compare-and-set: undo replays a decision taken seconds
    // ago, so the server must refuse it if the session moved on since.
    await waitFor(() => expect(mocks.setSlotFolder).toHaveBeenCalledWith(SLOT_KEY, null, ARCHIVE))
