...
/** Status dot tone shown on a rail row. `undefined` renders no dot. */
export type CrewRailTone = 'ok' | 'warn' | 'info'
...
export interface CrewEditorSection {
  ...
  count?: string
  /** Draws a status dot. Carries no text, so it never stands alone as meaning. */
  tone?: CrewRailTone
  /**
  ...
  return useMemo(() => {
    const rows: CrewEditorSection[] = [
      ...
      },
      {
        ...
        ...(sharesStorage ? { tone: 'info' as const } : {}),
      },
      {
        key: 'schedules',
        group: t('components.crewEditor.group_when_it_works'),
        icon: Clock,
