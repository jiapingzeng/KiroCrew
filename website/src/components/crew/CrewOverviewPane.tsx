...
export interface CrewOverviewPaneProps {
  ...
  routingWords: number
  /** Other crews pointing at this crew's workspace or memory store. */
  sharedWith: string[]
}
...
function Stat({ icon, value, label }: { icon?: React.ReactNode; value: string; label: string }) {
  return (
    ...
    <div className="min-w-[132px] flex-1 basis-full border-t border-border px-3 py-2 first:border-t-0
      ...
      <div className="mt-0.5 text-[9.5px] uppercase tracking-[0.06em] text-muted">{label}</div>
    </div>
  )
}
...
export default function CrewOverviewPane({
  ...
  resolvedModel, activeSchedules, schedulesUnknown, routingWords, sharedWith,
  ...
  const outputs: CrewWireNode[] = [
    ...
    },
    {
      ...
      mono: true,
      ...(sharedWith.length
        ? { tag: { tone: 'info' as const, text: t('components.crewEditor.tag_shared') } }
        : {}),
    },
    {
      key: 'memory',
    ...
    <div className="flex flex-col gap-3.5">
      <div className="flex flex-wrap rounded-lg border border-border bg-bg-accent">
        ...
        <Stat
          ...
          value={String(sharedWith.length)}
      ...
      <CrewOverviewDiagram
        inputs={inputs}
        outputs={outputs}
        inputsLabel={t('components.crewEditor.wire_who_wakes_it')}
