 ...
 */
export default function SessionMoveUndoBar({ moved, onUndo, compact = false }: Props) {
  useEffect(() => {
    ...
    >
      ...
      <div className="flex items-center gap-1.5 border-t border-border bg-accent-subtle px-2.5 py-1.5" role="status" aria-live="polite">
        ...
        <span
          ...
          title={`${i18nT('components.sessionMoveUndoBar.moved_to')} ${moved.toFolderName ?? i18nT('components.sessionMoveUndoBar.unfiled')} — ${moved.sessionTitle}`}
        >
          {!compact && <span className="shrink-0">{i18nT('components.sessionMoveUndoBar.moved_to')}</span>}
          {moved.toFolderName === null ? (
            <span className="truncate font-medium text-text-strong">{i18nT('components.sessionMoveUndoBar.unfiled')}</span>
          ) : (
            <>
      ...
      <motion.div
        ...
        initial={{ scaleX: 1 }}
        animate={{ scaleX: 0 }}
        transition={{ duration: MOVE_UNDO_MS / 1000, ease: 'linear' }}
      />
    </motion.div>
  )
}
