        ...
        {tokens.length === 0
          ...
          : tokens.map(token => (
            <RailRow
              ...
              dot={<Dot className={switchOn && token.enabled !== false ? 'bg-ok' : 'bg-muted'} />}
              title={token.label}
  ...
  const tokenPane = selectedToken && (
    <>
      ...
      <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border">
        ...
        <Badge variant={switchOn && selectedToken.enabled !== false ? 'ok' : 'muted'} className="ml-auto">
          {switchOn && selectedToken.enabled !== false
