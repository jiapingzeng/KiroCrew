...
function ChatSidebar({
  ...
  const undoDragMove = useCallback(() => {
    if (!dragMove) return
    // Compare-and-set against the destination this offer was armed for: another
    // client can have moved the session on before its broadcast reached us, and
    // an unconditional write would silently overwrite that newer placement.
    // The server refuses with 409 and the hook lands on its authoritative
    // folder instead. A `fromFolderId` whose folder was deleted meanwhile needs
    // no check here — the server degrades a CONDITIONAL write to a missing
    // folder to unfiled, which is the same answer with one mechanism instead of
    // two, and the only one that can see a deletion we have not heard about yet.
    assignToFolder(dragMove.slotKey, dragMove.fromFolderId, { expectFolderId: dragMove.toFolderId })
    setDragMove(null)
  }, [dragMove, assignToFolder])
  ...
  return (
    ...
    <div ref={sidebarRootRef} className="sidebar sidebar-inner bg-bg-elevated border border-border rounded-xl shadow-sm flex flex-col shrink-0 relative h-full" style={{ width: sidebarWidth }}>
      ...
      <AnimatePresence initial={false}>
        {dragMove?.live && (
          <SessionMoveUndoBar key={dragMove.id} moved={dragMove} onUndo={undoDragMove}
