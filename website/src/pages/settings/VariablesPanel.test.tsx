    ...
    it("surfaces the backend's reason for a refusal, not a generic failure", async () => {
      ...
      saveVariables.mockRejectedValue(
        new ApiError(
          ...
          JSON.stringify({
            error:
              'these variables come from config.local.json and cannot be changed here: REGION',
            code: 'variables_overlay_owned',
          }),
        ),
      )
      mount()
      ...
      expect(
        await screen.findByText(/come from config\.local\.json and cannot be changed here/),
      ).toBeInTheDocument()
    })
