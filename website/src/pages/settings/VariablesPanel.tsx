...
interface TableProps {
  ...
  noteId: string
  /** Names supplied by config.local.json: shown, but not editable from here. */
  overlayOwned?: string[]
}

function VariableTable({
  scope, pairs, winning, broader, onSave, busy, noteId, overlayOwned,
}: TableProps) {
  ...
  const owned = new Set(overlayOwned ?? [])
  ...
  const add = () => {
    ...
    if (bad) { setError(bad); return }
    setError(null)
    // The fields are NOT cleared here. A save can be refused — an overlay-owned
    // key, a reserved name the client grammar does not know, a gateway error — and
    // clearing on dispatch threw away what the user typed, leaving them an error
    // message and an empty form with nothing to correct or retry. The caller clears
    // them once the write has actually landed.
            ...
            return (
              <li key={name} className="flex flex-col gap-1 border-t border-border pt-2">
                ...
                <div className="text-[12px]">
                  ...
                  {owned.has(name) && (
                    <span className="block text-muted">{i18nT('pages.settings.variablesPanel.overlay_owned')}</span>
                  )}
                  {shadowed && (
...
export function VariablesPanel() {
  ...
  const saveMut = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      // still showed the generic message — including `variables_overlay_owned`,
      // the one refusal client-side validation cannot pre-empt, because a key owned
      // only by config.local.json is absent from `pairs` and never renders as a row.
      ...
      <SettingsSection title={i18nT('pages.settings.variablesPanel.global_environment_variables')}>
        <SettingsCard>
          ...
          <VariableTable
            ...
            overlayOwned={data.overlay_owned?.global}
          />
        </SettingsCard>
      </SettingsSection>
                ...
                return (
                  ...
                  <div key={ws} role="region" aria-label={i18nT('pages.settings.variablesPanel.workspace_named', { name: ws })} className="flex flex-col gap-2">
                    ...
                    <VariableTable
                      ...
                      overlayOwned={data.overlay_owned?.workspaces?.[ws]}
                    />
                  </div>
                )
              })}
