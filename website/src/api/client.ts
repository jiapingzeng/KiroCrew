...
export interface VariablesView {
   ...
   * it and the overlay would re-supply it on the next load. The panel renders such
   * a row read-only with a note naming the file, and the backend refuses a `set` or
   * `delete` naming one (400 `variables_overlay_owned`).
   */
  overlay_owned?: {
    global: string[]
 ...
 * the MERGED config and drew a data-loss finding in three consecutive review
 * rounds: a base value shadowed by `config.local.json` was invisible in that read
 * and got dropped, and two tabs replacing the same scope clobbered each other's
 * unrelated edits.
 */
export interface VariablesWrite {
   ...
   * back a map read from the MERGED config, which lost a base value shadowed by
   * `config.local.json` and let two tabs clobber each other's unrelated edits.
   *
   * A key `overlay_owned` reports is REFUSED (400 `variables_overlay_owned`):
   * `config.local.json` wins on load, so a base write would be inert.
   */
