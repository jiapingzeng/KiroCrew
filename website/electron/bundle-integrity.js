...
function describeIncompleteBundle(missing) {
  ...
  return `Kiro Crew's bundled Python runtime is still being installed — ${detail} `
    + "not on disk yet. Wait for the installer or update to finish, then retry. "
    + "If this persists after restarting the app, reinstall it.";
}
...
function isIncompleteBundleCrash(text) {
  ...
  // No marker in the tail means the boundary scrolled off; judging the whole tail
  // then risks reading a previous attempt, so decline and let the ordinary
  // failure path report it.
  const at = full.lastIndexOf(SPAWN_MARKER);
  if (at === -1) return false;
  const s = full.slice(at);
 ...
 * @param {boolean} [o.portInUseInLog]  the tail reports a bound port
 * @returns {boolean}
 */
